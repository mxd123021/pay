...
$("#js_preview_area").on("click",".js_edit_icon",function(){
  ...
  $('#js_edit_area .ckinput').each(function(){
    ...
    $('#'+thisid).keyup(function(){
	  ...
	  var tmplen=0;
	  if(!inputstr && (thisid=='promotionname' || thisid=='customurlname' || thisid=='custom_cell1name')){
 ...
 function checkInPut(){
  ...
  /*$('#send_name').keyup(function(){
	  ...
	  var strleng=inputstr.gbLen();
	  var tmplen=0;
      $('#send_name_preview').text(inputstr);
	  $('#js_send_name_tip').text(Math.ceil(strleng/2));
...
$('#sub_add_card').click(function(){
  ...
  $('#add_wx_card').click(function(){
	...
	$('#create_card').click(function(){
		if (card_flag) return false;
		if(checkInPut()){
			card_flag = true;
			var thisobj=$(this);
			if (parseInt($('#is_card').val()) > 0) {
				thisobj.prop('disabled',true);
				swal({
					title: "您确定创建新的红包？",   
					text: "如果您创建新的红包，已有的红包将被删除!",
					type: "warning",
