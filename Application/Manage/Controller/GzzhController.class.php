 ...
 * 公众账号支付
 */
class GzzhController extends BaseController
{
    ...
    public function callback()
    {
        ...
        if($this->resHandler->isTenpaySign())
        {
            if($this->resHandler->getParameter('status') == 0 && $this->resHandler->getParameter('result_code') == 0)
            {
                ...
                // 11;
                dataRecodes('in callback', $order_id);
                // //更改订单状态
        ...
        switch($method)
        {
            case 'submitOrderInfo'://提交订单
                ...
                */
                ...
                if(in_array($data['uid'], array(39, 139, 143, 161))){//金誉
                    $data['pmid'] = 178;
    ...
    {
        $data['trade_type'] = $trade_type = 'pay.weixin.jspay';
        $res = D('Manage/XyOrder')->addOrder($data);
//        dataRecodes('in submitOrderInfo', $data);
        if(!$res)
        {
        ...
        $this->reqHandler->setParameter('notify_url', U('Gzzh/index@' . C('SITE_URL'), array('method' => 'callback')));//
        ...
        if($this->pay->call()){
            ...
            if($this->resHandler->isTenpaySign()){
                ...
                if($this->resHandler->getParameter('status') == 0 && $this->resHandler->getParameter('result_code') == 0){
                    echo json_encode(array('status' => $this->resHandler->getParameter('status'), 'token_id'=>$this->resHandler->getParameter('token_id'), 'pay_url' => 'https://pay.swiftpass.cn/pay/jspay?token_id=' . $this->resHandler->getParameter('token_id') . '&showwxtitle=1'));
                    exit();
                }else{
