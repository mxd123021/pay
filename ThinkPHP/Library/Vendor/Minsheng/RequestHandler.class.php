...
class RequestHandler {
	...
	function RequestHandler() {
		$this->gateUrl = "'https://gzwkzf.cmbc.com.cn/payment-gate-web/gateway/api/backTransReq";
		$this->key = "";
	...
	*/
	function createSign() {
        ...
        function createMSSign(){
            ...
            //读取私钥文件
            //注意所放文件路径
            $priKey = file_get_contents(dirname ( __FILE__ ) .'/key/850440053991421_prv.pem');
            dataRecodes('私钥1', $priKey);
            //转换为openssl密钥，必须是没有经过pkcs8转换的私钥
            $res = openssl_get_privatekey($priKey);
            dataRecodes('私钥2', $res);
            //调用openssl内置签名方法，生成签名$sign
            openssl_sign($data, $sign, $res);
            dataRecodes('私钥3', $sign);
            //释放资源
            openssl_free_key($res);
            $sign = urlencode(base64_encode($sign));
            dataRecodes('私钥4', $sign);
